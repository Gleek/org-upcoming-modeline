...
- =org-upcoming-modeline-show-running= (off by default): count down the end
  of the time-block you are currently inside, and switch to the next one
  =org-upcoming-modeline-switch-ahead= seconds before it ends. See
...
- =org-upcoming-modeline-show-running= makes the modeline count down the
  ...
  #+begin_src emacs-lisp
    (setq org-upcoming-modeline-show-running t
          org-upcoming-modeline-switch-ahead (* 10 60))
  #+end_src
  Off by default. Running events are looked for up to
